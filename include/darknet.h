...
/** 
 * 网络layer结构类型（枚举类型），对应的整型值由CONVOLUTIONAL从0开始编号，共24中网络类型（最后一个对应的整型值为23）.
*/
typedef enum {
    CONVOLUTIONAL,
    DECONVOLUTIONAL,
...
struct layer
{
    LAYER_TYPE type;  // 网络层类型，枚举类型，取值参见LAYER_TYPE枚举类型的定义
    ACTIVATION activation; // 网络层所用的激活函数类型
    COST_TYPE cost_type; // 网络层所用的损失函数类型
    ...
    int batch_normalize; // 是否进行BN；值为1时进行BN
    int shortcut; // 是否直连
    int batch; // 一个batch中含有的图片张数，等于net.batch，详细可以参考network.h中的注释
               // 一般在构建具体网络层时赋值（比如make_maxpool_layer()中）
    int forced;
    int flipped;
    ...
    int outputs; // 该层对应一张输入图片的输出元素个数（一般在各网络层构建函数中赋值，比如make_connected_layer()）
                 // 对于一些网络，可由输入图片的尺寸及相关参数计算出，比如卷积层，可以通过输入尺寸以及步幅、核大小计算出；
                 // 对于另一些尺寸，则需要通过网络配置文件指定。如未指定，取默认值1，比如全连接层（见parse_connected()函数）
    int nweights;
    int nbiases;
    int extra;
    int truths; // 表示一张图片含有的真实值的个数。对于检测模型来说，一个真实的标签含有5个值，
                // 包括类型对应的编号以及定位矩形框用到的w,h,x,y四个参数。在YOLOv3中，固定每张图片最大处理90个矩形框（可查看max_boxes参数），
                // 默认每张图片可检测90个目标; l.truths = l.max_boxes*(4 + 1); 
    int h,w,c; // 该层输入图片的高度、宽度、通道数（一般在各网络层构建函数中赋值，比如make_connected_layer()），
                // 第一层网络的h,w,c就是网络初始能够的接收的图片尺寸，而后每一层的h,w,c都与自动匹配上一层相应的输出参数，
                 ...
                 // 对于全连接层，h和w置为1；c置为l.inputs（参见make_connected_layer()）
    int out_h, out_w, out_c; // 该层输出图片的高度、宽度、通道数（一般在各网络层构建函数中赋值，比如make_connected_layer()），
                                // 对于卷积层，可由上面的h,w,c以及卷积核大小、步幅计算出；对于全连接层，out_h,out_w的值置为1,
    ...
    int flatten;
    int spatial;
    int pad; // 对输入数据四周的补0长度（卷积层，最大池化层用到该参数）；一般在构建具体网络层时赋值（比如make_maxpool_layer()中）
    int sqrt;
    int flip;
    int index;
    int binary;
    ...
    int noloss;
    int softmax;
    int classes;  // 目标类别数。一个训练好的网络，只能检测指定所有目标类别中的目标，比如yolo9000.cfg，设置该值为9418，
                  // 也就是该网络训练好了之后可以检测9418种物体。该参数由网络配置文件指定。
    int coords;  // 这个参数一般用在检测模型中，且不是所有层都有这个参数，一般在检测模型最后一层有，
                 //该参数的含义是定位一个目标所需的参数个数，一般为4个，包括目标所在矩形框中心坐标x,y两个参数以及矩形框长宽w,h两个参数，
    ...
    int onlyforward;
    int stopbackward; // 用来强制停止反向传播过程（值为1则停止反向传播），参看network.c中的backward_network()函数
    int dontload;
    int dontsave;
    int dontloadscales;
    int numload;
    ...
    float scale; // 在dropout层中，该变量是一个比例因子，取值为保留概率的倒数（darknet实现用的是inverted dropout），用于缩放输入元素的值
                // inverted dropout在训练阶段期间对激活值进行缩放，而测试阶段保持不变。在make_dropout_layer()函数中赋值
    ...
    int   * indexes; 	// 维度为l.out_h * l.out_w * l.out_c * l.batch，可知包含整个batch输入图片的输出，一般在构建具体网络层时动态分配内存（比如make_maxpool_layer()中）。
						...
						// 这些局部最大元素值在上一层所有输出元素中的总索引。记录这些值可以用于反向传播过程计算上一层敏感度值，详见backward_maxpool_layer()以及forward_maxpool_layer()函数。
    int   * input_layers;
    int   * input_sizes;
	 /** 
     * 这个参数仅在region_layer.c中使用，用于不同数据集之间类别编号的转换，更为具体的，
     * 是coco数据集中80类目标编号与联合数据集中9000+目标类别编号之间的转换，可以对比查看data/coco.names与
     * data/9k.names以及data/coco9k.map三个文件。
     * 可以发现，coco.names中每一个目标类别都可以在9k.names中找到, 且coco.names中每个目标类别名称在9k.names
     * 中所在的行数就是coco9k.map中的编号值，也就是这个map将coco数据集中
     * 的类别编号映射到联合数据集9k中的类别编号（这个9k数据集是一个联合多个数据集的大数集，其名称分类被层级划分，
     * ）（注意两个文件中物体的类别名称大部分都相同，有小部分存在小差异，虽然有差异，但只是两个数据集中使用的名称有所差异而已，
     * 对应的目标是一样的，比如在coco.names中摩托车的名称为motorbike，在联合数据集9k.names，其名称为motorcycle）.                   
    */
    int   * map;
    int   * counts;
    float ** sums;
    float * rand; // 该参数目前只发现用在dropout层，用于存储一些列的随机数，这些随机数与dropout层的输入元素一一对应，维度为l.batch*l.inputs（包含整个batch的），在make_dropout_layer()函数中用calloc动态分配内存，
                  // 并在前向传播函数forward_dropout_layer()函数中逐元素赋值。里面存储的随机数满足0~1均匀分布，用于决定该输入元素的去留，
                  // dropout层完成的一件事是：按照一定概率舍弃输入神经元（所谓舍弃就是置该输入的值为0），rand中存储的值就是如果小于l.probability，则舍弃该输入神经元（详见：forward_dropout_layer()）。
    ...
    float * cost; // 损失函数值，用于计算最后的cost，比如识别模型中的cost_layer层，
                  // 检测模型中的region_layer层
    float * state;
    float * prev_state;
    float * forgot_state;
    ...
    float * weight_updates;  // 当前层所有权重系数更新值，对于卷积层维度为l.n*l.c*l.size*l.size；对于全连接层，维度为单张图片输入与输出元素个数之积inputs*outputs，
							// 权重系数更新值就是梯度下降中与步长相乘的那项，也即误差对权重的导数。一般在各网络构建函数中动态分配内存（比如make_connected_layer()

    float * delta; 	// 存储每一层的误差，或称为敏感度图：包含所有输出元素的敏感度值（整个batch所有图片）。所谓敏感度，即误差函数关于当前层每个加权输入的导数值。
    ...
    tree *softmax_tree; // softmax层用到的一个参数。仅发现darknet9000.cfg中使用了该参数，如果未用到该参数，其值为NULL，如果用到了则会在parse_softmax()中赋值，
                                  

    size_t workspace_size; 	// net.workspace的元素个数，为所有层中最大的l.out_h*l.out_w*l.size*l.size*l.c
...
typedef struct network
{
    ...
    int batch;    // parse_net_options()中赋值：一个batch含有的图片张数，下面还有个subdivisions参数 
                  // batch*subdivision作为输入一个GPU的图像数目
    size_t *seen; // 目前已经读入的图片张数（网络已经处理的图片张数）（在make_network()中动态分配内存）
    int *t;
    float epoch;
    int subdivisions; // parse_net_options()中赋值 
    ...
    int time_steps;
    int step;
    int max_batches; //训练的最大batch数目
    float *scales;
    int   *steps;    //学习率调整steps
    int num_steps;   //学习率调整steps数目
    int burn_in;
    ...
    int outputs;    // 一张输入图片对应的输出元素个数，对于一些网络，可由输入图片的尺寸及相关参数计算出，比如卷积层，可以通过输入尺寸以及步幅、核大小计算出；
	...
	float *input;      // 中间变量，用来暂存某层网络的输入（包含一个batch的输入），比如某层网络完成前向处理，将其输出赋给该变量，作为下一层的输入，可以参看network.c中的forward_network()与backward_network()两个函数，
	...
	float *delta;	   // 中间变量，用来暂存某层网络的误差，即敏感度图（反向传播处理当前层时，用来存储上一层的敏感度图，因为当前层会计算部分上一层的敏感度图，可以参看network.c中的backward_network()函数），
...
typedef struct 
{
	...
	int h;        // 每张图片的高度（行数）
    int c;        // 每张图片的通道数
    float *data;  // 数据头指针，存储二维的图像数据，按行存储，如果有多张，则每张按行存储，然后所有图片再并成一大行
} image;
...
typedef struct
{
    ...
    matrix X;
    matrix y;
	// shallow是指深层释放x,y中vals的内存还是浅层释放x,y中vals（注意是x,y的vals元素，不是x,y本身，x,y本身是不能用free释放的，因为其连指针都不是）的内存
    // x,y的vals是一个二维数组，如果浅层释放，即直接释放free(vals)，这种释放并不会真正释放二维数组的内容，因为二维数组实际是指针的指针，
    // 这种直接释放只是释放了用于存放第二层指针变量的内存，也就导致二维数组的每一行将不再能通过二维数组名来访问了
    //（因此这种释放，需要先将数据转移，使得有其他指针能够指向这些数据块，不能就会造成内存溢出了）；
...
typedef struct{
    ...
    float x,y,w,h; // 矩形中心点的x,y坐标，以及矩形宽高w,h（其值不是真实的像素坐标，而是相对输入图片的宽高比例）
    float left, right, top, bottom; // 矩形四个角点的最大最小x,y坐标（其值不是真实的像素坐标，而是相对输入图片的宽高比例）
} box_label;
