...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    // 可通过改写voc_label.py指定生成文件目录。小规模数据集也可以自己手工制作。
...
#endif
        nets[i] = load_network(cfgfile, weightfile, clear); //## 加载网络模型
 ...
 * @param filename      待检测的图片路径（单张图片）
 * @param thresh        阈值，类别检测概率大于该阈值才认为其检测结果有效
 * @param hier_thresh   
 * @param outfile
 * @param fullscreen
 * @details 
 * 说明：该函数为一个前向推理测试函数，不包括训练过程。因此如果要使用该函数，必须提前训练好网络，并加载训练好的网络参数文件。
 *     本函数由darknet.c中的主函数调用。
 * 说明２：该函数包含了一个端到端的过程。
 * 　　　　即初始化模型、读取模型权重、读取图片、图片resize、将图片喂给模型进行前向传播、
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
	...
	//## 加载之前训练的超参（权重等）。参见network.c中的详细注释。
    network *net = load_network(cfgfile, weightfile, 0);  
    ...
    while(1)
    {
		...
		//## 调整图片尺寸
		//输入图片的尺寸调整为神经网络能够处理的图片尺寸net.w、net.h 
