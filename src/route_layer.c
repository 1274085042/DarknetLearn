...
route_layer make_route_layer(int batch, int n, int *input_layers, int *input_sizes)
{
    ...
    l.batch = batch;
    l.n = n;
    l.input_layers = input_layers; //可能有多个input_layers
...
void backward_route_layer(const route_layer l, network net)
{
    ...
    for(i = 0; i < l.n; ++i){
        ...
        for(j = 0; j < l.batch; ++j){
			// y=ALPHA*x+y;
	        // ALPHA=1; x=l.delta + offset + j*l.outputs; y= delta + j*input_size
            axpy_cpu(input_size, 1, l.delta + offset + j*l.outputs, 1, delta + j*input_size, 1); //求和
        }
        offset += input_size;
    }
}
