...
**      比如在训练voc数据时，输入的train.txt文件中只包含所有图片的具体路径，如：/home/bai/darknet/VOCdevkit/VOC2007/JPEGImages/000001.jpg
**      而000001.jpg的标签并没有给, 程序通过该函数替换掉图片路径中的JPEGImages为labels，并替换掉后缀.jpg为.txt得到的，最终得到：
**      /home/bai/darknet/VOCdevkit/VOC2007/labels/000001.txt
...
/** 找出数组a中的最大元素，返回其索引值.
 * @param a 一维数组，比如检测模型中，可以是包含属于各类概率的数组（数组中最大元素即为目标最有可能所属的类别）
 * @param n a中元素的个数，比如检测模型中，a中包含所有的目标类别，此时n为物体类别总数
 */
