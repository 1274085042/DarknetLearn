...
**        在train_detector()中调用，目的是从train.txt中读入所有训练图片的路径（文件中每一行就是一张图片的全路径）
*/
...
/*
** 读入一张图片的所有box：一张图片可能有多个目标，每个目标都有一个矩形框框起来（目标检测不单识别类别，更包括定位），
** 本函数就是读入一张图片的所有box信息。每个box包括5条信息，依次为：目标类别id，矩形中心点x坐标，矩形中心点y坐标，
** 矩形框宽度w, 矩形框高度h。
** 输入： filename    标签数据所在路径（标签数据需要下载，然后调用voc_label.py生成指定的格式，具体路径视情况而定）
...
box_label *read_boxes(char *filename, int *n)
{
    ...
    while(fscanf(file, "%d %f %f %f %f", &id, &x, &y, &w, &h) == 5){
        ...
        boxes[count].h = h;
        boxes[count].w = w;
		// 通过x,y,w,h计算矩形框四个角点的最小和最大的x,y坐标
...
void correct_boxes(box_label *boxes, int n, float dx, float dy, float sx, float sy, int flip)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        // 要搞清一个概念，最初的矩形框的x,y,w,h是相对于输入的训练图片的宽高比例值，因此矩形框必须在图片内，就算有一个目标，图片没有照全，
        // 那么给的矩形框也必须是图片内的矩形框，这时矩形框只覆盖目标的部分内容，总之不可能出现矩形框中心坐标为(0,0)（或者为负）的情况。
...
/*
** 用来获取一张图的真实标签信息，对于图像检测，标签信息包括目标的类别（用类别id表示）以及定位信息，定位用矩形框来表示，包含矩形中心点坐标x,y以及宽高w,h，
** 本函数读入一张图片中所有标签信息（一张图片可能存在多个目标，每个目标都含有一条类别信息以及一个矩形框信息）
...
void fill_truth_detection(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy)
{
    ...
    for (i = 0; i < count; ++i) 
    {
        ...
        w =  boxes[i].w;
        h =  boxes[i].h;
        id = boxes[i].id; // 目标的类别并不是用字符串来表示，而是用物体类别对应的id来表示，如对于VOC数据集，共有20类物体，那么对应的id编号为从0~19号
...
{
    // 从包含数据集中所有物体类别信息的文件（data/**.names文件）中读取所有目标类别信息，存入链表plist中
    ...
    // TODO:这里或许可以额外写个函数直接将数据读取至labels，没有必要调用list_to_array先读到plist，而后转存至labels，
...
** 从所有训练图片中，读取n张，并对这n张图片进行数据增强，同时矫正增强后的数据标签信息。最终得到的图片的宽高为w,h（原始训练集中的图片尺寸不定），也就是网络能够处理的图片尺寸，
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    ...
    // n = net.batch * net.subdivisions * ngpus
    // 从parse_net_option()函数可知，net.batch = net.batch / net.subdivision，等号右边的那个batch就是
    // 网络配置文件.cfg中设置的每个batch的图片数量，又引入了subdivision这个参数。总之，
...
{
    pthread_t thread;
	// 同样第一件事深拷贝了args给ptr（为什么每次都要做这一步呢？估计是多线程处理的需要）
...
void *load_threads(void *ptr)
{
	...
	// free(ptr)将使得ptr不能再访问非指针变量（如int h等，实际经过测试，在gcc编译器下，能访问但是值被重新初始化为0），
	...
	//		而至于free(ptr)还会不会做其他事情，比如会不会重新初始化这块内存为0（擦写掉），以及怎么擦写，这些操作，是不确定的，可能跟具体的编译器有关（个人猜测），
...
** 说明： darknet作者在实现读入图片数据       总的流程是：load_data(load_args args)->load_threads(load_args* ptr)->load_data_in_thread(load_args args)->load_thread(load_args* ptr),
**       load_thread()函数中会选择具体的读入函数，比如load_data_detection()，要强调的是，所有这些函数，都会输入load_args类型参数，或者是其指针，
**       而且很多函数开头就会新建一个args，并且深拷贝传入的args，而后返回新建的args，同时可能会随意改动甚至释放传入的args。
**       这一整串流程要记住：不管args在之后的函数中怎么变，       其中的指针变量所指的内存块其实都是一样的
**      （所以不管在哪个函数中，不要随意深层释放args.d的数据，除非是真正的用完了，而这里还处于数据读入阶段，所以决不能深层释放args.d）；
**       因为load_data()函数中传入的args是按值传递（不是指针）的，不管之后的函数怎么改动args的非指针变量，都不会影响load_data()函数外的值。
*/
pthread_t load_data(load_args args)
{
    ...
    struct load_args *ptr = calloc(1, sizeof(struct load_args));
    *ptr = args;
	 // 创建相应线程，并绑定load_threads()函数到该线程上，第二个参数是线程的属性，这里设置为0（即NULL）,第四个参数ptr就是load_threads()的输入参数
...
** 碎碎念（TODO）：为什么总是动不动返回或者传入data等其它结构体实例呢，传指针不是更快么:(
*/
...
**      而后再调用train_network()，总之最后train_network()的输入d中只包含net.batch*net.subdivision张图片），可知本函数只是获取其中一个小batch图片，事实上，
