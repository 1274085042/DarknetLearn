...
/*
**  读取数据配置文件（.data文件），包含数据所在的路径、名称，其中包含的目标类别数等
**  返回：list指针，包含所有数据信息。函数中会创建options变量，并返回其指针
**       若文件打开失败，将直接退出程序，不会返空指针
*/
...
**  Using default...字样的提示，因此叫做quiet（就是使用默认值，没有提示）
*/
