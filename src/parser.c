...
** 因此，key和val，只能用free()释放key的内存，而val是不行的。
...
{
	// 读取网络模型的结构，存于双向链表中
	// 从神经网络结构参数文件中读入所有神经网络层的结构参数，存储到链表sections中，
