...
* 网络前向推理函数
* 前向计算网络net每一层的输出.
* @param netp 构建好的整个网络模型
* 说明： 遍历net的每一层网络，从第1层到最后一层，逐层计算每层的输出
*/
...
void backward_network(network *netp)
{
    ...
    for(i = net.n-1; i >= 0; --i)
    {
        ...
        if(i == 0)
        {
			// i = 0时，也即已经到了网络的第1层,        就是直接与输入层相连的第一层隐含层
			// 对于第1层来说，其前面已经没有网络层了，因此没有必要再计算前一层的参数
			// 故没有必要再获取上一层；也就不需要计算上一层的delta。
			// 第一层的输入就是图像输入，也即整个net最原始的输入，
			// 在开始进行反向传播之前，已经用orig变量保存了
            net = orig;
        }
        else
        {
			...
			// 上一层的输出作为当前层的输入。下面l.backward()会用到
			// 具体是在计算当前层权重更新值时要用到
            net.input = prev.output;

			// 上一层的error,即敏感度图（l.backward()会同时计算上一层的敏感度图）
...
/*
* 对一个batch数据进行训练网络（含前向推理和后向传播）
*/
float train_network_datum(network *net)
{
	// 更新目前已经处理的图片数量：每次处理一个net->batch，故添加net->batch
    *net->seen += net->batch;
...
float *network_predict(network *net, float *input)
{
    ...
    net->train = 0;
    net->delta = 0;
    forward_network(net); //调用网络前向预测函数
