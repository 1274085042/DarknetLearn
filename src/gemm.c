...
**		  ldc	  C的列数
**	说明：此函数是用C实现矩阵乘法运算，这部分代码模仿Caffe中的math_functions.cpp的代码
**		 参考博客：http://www.voidcn.com/blog/thy_2014/article/p-6149690.html
**		 更为详细的注释参见：gemm_cpu()函数的注释
...
**			 举个例子，假设： A = [1, 2, 3, 2, 2, 1], B = [2, 0, 1, 1, 2, 1], C = [3, 0, 1, 2] （这些输入都是一维数组格式），且C为2*2的矩阵，即C = [3, 0; 1, 2]，
**           那么要进行C = ALPHA * A * B + BETA * C的计算，
...
**			 对于gemm_nt()与gemm_tt()，与上分析一样，不再赘述了。此部分注释可自己写程序测试。
...
**    所以下面会有一些不同的操作（由于这个原因，相比于cpu版本的gemm_cpu()，又要复杂了一些，下面举例说明）
...
**       会看出这是按列存储的方式：也即a的前两个数是A_gpu'的第一列，再后两个数是A_gpu’的第二列，依次类推，B_gpu’也一样，
